...
Paths:
  - Input:  ../1-GatheringPreprocessingTransformation/GeneratedDataFiles/all_games_merged_clean.csv
  - Output: ../0-Data/week_<N>_games.csv   (one file per calendar week of games)
"""
...
def _default_output_path(script_dir: str) -> str:
    # Weekly output files live in the project-level 0-Data folder.
    return os.path.normpath(
        os.path.join(
            script_dir,
            "..",
...
def create_per_second_weekly_files(
    input_path: Optional[str] = None,
    output_path: Optional[str] = None,
    max_games: Optional[int] = None,
) -> int:
    """
    Build per-second, two-team-pivoted rows from `all_games_merged_clean.csv`
    and write one CSV per calendar week (week_<N>_games.csv) into the folder
    containing `output_path`. Returns the number of weekly files written.
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    inp = input_path or _default_input_path(script_dir)
    out = output_path or _default_output_path(script_dir)

    if not inp.startswith("/"):
        inp = os.path.normpath(os.path.join(script_dir, inp))
    ...
    if not os.path.exists(inp):
        print(f"Input file not found; skipping per-second expansion: {inp}")
        return 0

    df = pd.read_csv(inp)
    if df.empty:
        print(f"Input is empty; no weekly files written: {inp}")
        return 0
    ...
    for week_idx, week_start in enumerate(
        ...
        print(
            f"\nProcessing week {pd.Timestamp(week_start).strftime('%Y-%m-%d')} "
            f"({len(week_events)} games)"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Base output path; weekly files are written into its directory",
    )
    parser.add_argument(
        "--max-games",
        type=int,
    ...
    create_per_second_weekly_files(
