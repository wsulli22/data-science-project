...
def get_kalshi_game_data(event_ticker: str, team_abbreviation: str) -> pd.DataFrame:
    ...
    # Kalshi API: newer responses use PriceDistribution with *_dollars strings
    # (e.g. close_dollars "0.5600" = 56% yes prob). Legacy used cents in close/mean/open/previous.
    rows = []
    for candle in candlesticks:
        ...
        win_prob_close = _price_to_prob(price, "close")
        win_prob_mean = _price_to_prob(price, "mean")
        win_prob_open = _price_to_prob(price, "open")
        win_prob_previous = _price_to_prob(price, "previous")

        vol = candle.get("volume")
        if vol is None and candle.get("volume_fp") is not None:
            try:
                vol = float(str(candle.get("volume_fp", "0")))
            except (TypeError, ValueError):
                vol = 0
        if vol is None:
            vol = 0

        oi = candle.get("open_interest")
        if oi is None and candle.get("open_interest_fp") is not None:
            try:
                oi = float(str(candle.get("open_interest_fp", "0")))
            except (TypeError, ValueError):
                oi = 0
        if oi is None:
            oi = 0
        ...
        rows.append({
            ...
            "win_prob_open": win_prob_open,
            "win_prob_previous": win_prob_previous,
            "volume": vol,
            "open_interest": oi,
            "result": result,
    ...
    # Forward-fill: no trade in period → use previous; then mean/open as fallbacks
    df["win_prob"] = (
        df["win_prob_close"]
        .fillna(df["win_prob_previous"])
        .fillna(df["win_prob_mean"])
        .fillna(df["win_prob_open"])
    )
    df["win_prob"] = df["win_prob"].ffill().bfill()
...
def _dollars_str_to_prob(s) -> float | None:
    """Kalshi FixedPointDollars, e.g. '0.5600' → probability 0.56."""
    if s is None or s == "":
        return None
    try:
...
def _price_to_prob(price: dict, field: str) -> float | None:
    """
    Read probability from price object. Supports current API (*_dollars) and legacy (cents).
    field: 'close' | 'mean' | 'open' | 'previous'
    """
    dollar_key = {"close": "close_dollars", "mean": "mean_dollars", "open": "open_dollars", "previous": "previous_dollars"}[field]
    p = _dollars_str_to_prob(price.get(dollar_key))
    if p is not None:
        return p
    return _cents_to_prob(price.get(field))
