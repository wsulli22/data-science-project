...
_viz_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "2-PreliminaryAnalysis"))
...
from build_weekly_data_files import create_per_second_weekly_files
...
NUM_GAMES_TO_ANALYZE = 10000000000 #USE A REALLY BIG NUMBER FOR ALL GAMES
NEWEST_GAME_DATE_CUTOFF_DATE = "2026-03-17"
OVERTIME_GAMES = "all"  # "all" or "only" (runs with ot_period > 0 only)
...
map_kalshi_and_espn_game_ids(limit=NUM_GAMES_TO_ANALYZE, overtime_games=OVERTIME_GAMES)

#FOR EACH SUCCESSFUL GAME MATCH, FETCH ESPN AND KALSHI DATA, MERGE, AND SAVE
fetch_and_merge_all_games(
    num_games=NUM_GAMES_TO_ANALYZE,
...
#EXPAND MERGED DATA TO ONE ROW PER SECOND AND WRITE WEEKLY FILES TO ../0-Data/
