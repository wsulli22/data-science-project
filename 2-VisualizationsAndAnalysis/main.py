...
MERGED_INPUT_CSV = os.path.join(
    PROJECT_DIR,
    "1-GatheringPreprocessingTransformation/GeneratedDataFiles/all_games_merged_clean.csv",
)
_PREV_CWD = os.getcwd()
try:
    ...
    # Signed edge: raw cell aggregates, then LinearGAM smooth (not smoothed calibration CSV)
    generate_smoothed_data_edge_heat_map(input_file=MERGED_INPUT_CSV, num_time_bins=40)
finally:
    os.chdir(_PREV_CWD)
