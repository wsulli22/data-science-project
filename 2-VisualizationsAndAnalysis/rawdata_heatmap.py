...
def generateHeatMap(input_file="GeneratedDataFiles/all_games_merged_clean.csv", num_time_bins=40):
    """
    ...
    Args:
        ...
        num_time_bins: Number of time bins (0-2400 seconds regulation).
                       Defaults to 40 (1 minute per bin).
    ...
    FILE = input_file
    # Write next to the other visualization outputs (stable regardless of cwd).
    script_dir = os.path.dirname(__file__)
    generated_data_dir = os.path.join(script_dir, "GeneratedDataAndVisualizations")
    ...
    for r in win_rate_matrix.index:
        for c in win_rate_matrix.columns:
            ...
            if pd.isna(wr) or n < MIN_OBS:
                ...
                annot_matrix.loc[r, c] = f"{wr*100:.2f}% ({int(n)})"

    # ── plot ─────────────────────────────────────────────────────────────
    # Scale figure width with number of time bins so that doubling bins
    # doubles the visual heatmap width.
    ...
    # Show every 5th x-axis label for better readability with 40 bins
    x_tick_positions = ax.get_xticks()
    x_tick_labels = [label.get_text() for label in ax.get_xticklabels()]
    # Keep every 5th label
