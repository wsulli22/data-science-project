...
2. Converts Kalshi team abbreviations to ESPN abbreviations via a manual dictionary (`teamConversionDict.py`).
...
4. **Discarded:** games with no ESPN match (team abbreviation not in the dictionary, or no ESPN game with those teams on that date) → logged as `"no_espn_match"`.

Successful mappings are written to `GeneratedDataFiles/kalshi_espn_game_mappings.csv` as `(kalshi_game_id, espn_game_id)` pairs. Unmatched games go to `unmatched_games.txt`.
...
#### Step 5: Generate the calibration heat map
**(`make_heat_map.py`)**

- Reads the merged CSV.
- **X-axis (time):** `game_elapsed_seconds` is bucketed into 8 bins of 5 minutes (300 s each): 0–5 min, 5–10 min, …, 35–40 min.
