...
OUTPUT_DIR = "GeneratedDataAndVisualizations"  # For CSV data files
VISUALIZATION_DIR = "GeneratedDataAndVisualizations"  # For PNG visualization files
...
def plot_heatmap_from_matrix(win_rate_matrix, count_matrix=None, df_raw=None, out_dir=OUTPUT_DIR, vis_dir=VISUALIZATION_DIR, save_csv=False):
    """
    ...
    Args:
        ...
        df_raw: Optional raw dataframe for calculating distribution percentages
    ...
    # ── Calculate distribution percentage matrix if raw data is available (needed for CSV and annotations) ──────────────────────
    ...
    if save_csv:
        ...
        # Remove any old "_pct" columns (without "distribution") if they exist
        old_pct_cols = [col for col in df_out.columns if col.endswith("_pct") and "distribution" not in col]
        if old_pct_cols:
            df_out = df_out.drop(columns=old_pct_cols)
        
        # Add distribution percentage columns if available
        if dist_pct_matrix is not None:
            # Create percentage columns with "_distribution_pct" suffix
            pct_df = dist_pct_matrix.copy()
            pct_df.columns = [f"{col}_distribution_pct" for col in pct_df.columns]
            # Align indices to ensure they match
            pct_df = pct_df.reindex(df_out.index, fill_value=0.0)
            # Combine with existing data
            df_out = pd.concat([df_out, pct_df], axis=1)
        
        # Reorder columns so each time bin is followed by its count and distribution percentage
        new_columns = []
        for time_bin in win_rate_matrix.columns:
            new_columns.append(time_bin)
            if count_matrix is not None:
                count_col = f"{time_bin}_count"
                if count_col in df_out.columns:
                    new_columns.append(count_col)
            if dist_pct_matrix is not None:
                dist_col = f"{time_bin}_distribution_pct"
                if dist_col in df_out.columns:
                    new_columns.append(dist_col)
        # Only reorder if all columns exist
        if all(col in df_out.columns for col in new_columns):
...
def generate_smoothed_heatmap(gam, df, out_dir=OUTPUT_DIR, vis_dir=VISUALIZATION_DIR):
    ...
    # Generate the plot and save CSV (pass df for distribution percentage calculation)
    return plot_heatmap_from_matrix(win_rate_matrix, count_matrix, df_raw=df, out_dir=out_dir, vis_dir=vis_dir, save_csv=True)
...
def load_smoothed_data_from_csv(csv_path):
    ...
    # Filter out count and distribution columns (columns ending with "_count" or "_distribution_pct")
    prob_columns = [col for col in df.columns if not col.endswith("_count") and not col.endswith("_distribution_pct")]
    df = df[prob_columns]
...
def main():
    ...
    if args.JustGraph:
        ...
        plot_heatmap_from_matrix(smoothed_matrix, count_matrix, df_raw=df_raw, out_dir=OUTPUT_DIR, vis_dir=VISUALIZATION_DIR, save_csv=True)
    else:
        ...
        if os.path.exists(csv_path):
            ...
            plot_heatmap_from_matrix(smoothed_matrix, count_matrix, df_raw=df_raw, vis_dir=VISUALIZATION_DIR, save_csv=False)
        else:
